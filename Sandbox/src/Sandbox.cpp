...
public:
	...
	virtual void OnAttach() override
	{
		...
		m_Mesh.reset(new Jasmine::Mesh("assets/meshes/cerberus.fbx"));
		m_SphereMesh.reset(new Jasmine::Mesh("assets/models/Sphere.fbx"));
	...
	virtual void OnUpdate() override
	{
		...
		Jasmine::UniformBufferDeclaration<sizeof(mat4), 1> quadShaderUB;
		...
		Jasmine::UniformBufferDeclaration<sizeof(mat4) * 2 + sizeof(vec3) * 4 + sizeof(float) * 8, 14> simplePbrShaderUB;
		...
		if (m_Scene == Scene::Spheres)
		{
			...
			for (int j = 0; j < 8; j++) {
				for (int i = 0; i < 8; i++)
				{
					m_SimplePBRShader->SetMat4("u_ModelMatrix", translate(mat4(1.0f), vec3(x, j * 22.0f, 0.0f)));
					m_SimplePBRShader->SetFloat("u_Roughness", roughness);
					m_SimplePBRShader->SetFloat("u_Metalness", metalness);
					m_SphereMesh->Render();
	...
	{
		if (event.GetEventType() == Jasmine::EventType::MouseScrolled) {
			auto* scrolled = (Jasmine::MouseScrolledEvent*)&event;
			m_Camera.MouseZoom(scrolled->GetYOffset() * 150.0f);
		}
	}
private:
	std::unique_ptr<Jasmine::Shader> m_Shader;
	std::unique_ptr<Jasmine::Shader> m_PBRShader;
	std::unique_ptr<Jasmine::Shader> m_SimplePBRShader;
	std::unique_ptr<Jasmine::Shader> m_QuadShader;
	std::unique_ptr<Jasmine::Shader> m_HDRShader;
	std::unique_ptr<Jasmine::Mesh> m_Mesh;
	std::unique_ptr<Jasmine::Mesh> m_SphereMesh;
	std::unique_ptr<Jasmine::Texture2D> m_BRDFLUT;

	struct AlbedoInput
	{
		glm::vec3 Color = { 0.972f, 0.96f, 0.915f }; // Silver, from https://docs.unrealengine.com/en-us/Engine/Rendering/Materials/PhysicallyBased
		std::unique_ptr<Jasmine::Texture2D> TextureMap;
		bool SRGB = true;
	...
	std::unique_ptr<Jasmine::VertexBuffer> m_VertexBuffer;
	std::unique_ptr<Jasmine::IndexBuffer> m_IndexBuffer;
	std::unique_ptr<Jasmine::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;
	...
	// Editor resources
	std::unique_ptr<Jasmine::Texture2D> m_CheckerboardTex;
};
