...
namespace Jasmine {
	...
	VulkanImage2D::~VulkanImage2D()
	{
		if (m_Info.Image)
		{
			...
			Renderer::Submit([info]()
			{
				...
				JM_CORE_WARN("VulkanImage2D::Release ImageView = {0}", (const void*)info.ImageView);
			});
		}
	}
	...
	{
		JM_CORE_ASSERT(m_Specification.Layers > 1);
	...
	void VulkanImage2D::UpdateDescriptor()
	{
		...
		//JM_CORE_WARN("VulkanImage2D::UpdateDescriptor to ImageView = {0}", (const void*)m_Info.ImageView);
	}
