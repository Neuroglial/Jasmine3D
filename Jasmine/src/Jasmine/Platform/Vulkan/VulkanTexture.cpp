...
namespace Jasmine {
	...
	namespace Utils {
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			JM_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			JM_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
		...
		: m_Path(path), m_Properties(properties)
	{
		...
		JM_CORE_ASSERT(m_ImageData.Data, "Failed to load image!");
		if (!m_ImageData.Data)
			return;
		...
		JM_CORE_ASSERT(m_Format != ImageFormat::None);
		...
		: m_Format(format), m_Properties(properties)
	{
		...
		JM_CORE_ASSERT(format == ImageFormat::RGBA);
	...
	void VulkanTexture2D::Invalidate()
	{
		...
		JM_CORE_ASSERT(m_ImageData.Data);
	...
	VulkanTextureCube::~VulkanTextureCube()
	{
		...
		{
			JM_CORE_TRACE("Destroying VulkanTextureCube");
