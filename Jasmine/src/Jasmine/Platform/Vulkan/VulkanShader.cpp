...
namespace Jasmine {
	...
	void VulkanShader::Reflect(VkShaderStageFlagBits shaderStage, const std::vector<uint32_t>& shaderData)
	{
		...
		JM_CORE_TRACE("===========================");
		JM_CORE_TRACE(" Vulkan Shader Reflection");
		JM_CORE_TRACE(" {0}", m_AssetPath);
		JM_CORE_TRACE("===========================");

		spirv_cross::Compiler compiler(shaderData);
		auto resources = compiler.get_shader_resources();

		JM_CORE_TRACE("Uniform Buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			JM_CORE_TRACE("  {0} ({1}, {2})", name, descriptorSet, binding);
			JM_CORE_TRACE("  Member Count: {0}", memberCount);
			JM_CORE_TRACE("  Size: {0}", size);
			JM_CORE_TRACE("-------------------");
		}

		JM_CORE_TRACE("Push Constant Buffers:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			JM_CORE_TRACE("  Name: {0}", bufferName);
			JM_CORE_TRACE("  Member Count: {0}", memberCount);
			JM_CORE_TRACE("  Size: {0}", bufferSize);
	...
	void VulkanShader::CreateDescriptors()
	{
		...
		for (uint32_t set = 0; set < m_ShaderDescriptorSets.size(); set++)
		{
			...
			for (auto& [binding, imageSampler] : shaderDescriptorSet.ImageSamplers)
			{
				...
				JM_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
			...
			for (auto& [bindingAndSet, imageSampler] : shaderDescriptorSet.StorageImages)
			{
				...
				JM_CORE_ASSERT(shaderDescriptorSet.UniformBuffers.find(binding) == shaderDescriptorSet.UniformBuffers.end(), "Binding is already present!");
				JM_CORE_ASSERT(shaderDescriptorSet.ImageSamplers.find(binding) == shaderDescriptorSet.ImageSamplers.end(), "Binding is already present!");
			...
			JM_CORE_INFO("Creating descriptor set {0} with {1} ubos, {2} samplers and {3} storage images", set,
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set)
	{
		...
		JM_CORE_ASSERT(m_TypeCounts.find(set) != m_TypeCounts.end());
	...
	VulkanShader::ShaderMaterialDescriptorSet VulkanShader::CreateDescriptorSets(uint32_t set, uint32_t numberOfSets)
	{
		...
		JM_CORE_ASSERT(poolSizes.find(set) != poolSizes.end());
	...
	{
		JM_CORE_ASSERT(set < m_DescriptorSetLayouts.size());
		ShaderMaterialDescriptorSet result;
	...
	{
		JM_CORE_ASSERT(set < m_ShaderDescriptorSets.size());
		JM_CORE_ASSERT(m_ShaderDescriptorSets[set]);
		if (m_ShaderDescriptorSets.at(set).WriteDescriptorSets.find(name) == m_ShaderDescriptorSets.at(set).WriteDescriptorSets.end())
		{
			JM_CORE_WARN("Shader {0} does not contain requested descriptor set {1}", m_Name, name);
			return nullptr;
		}
		...
		for (auto [stage, source] : m_ShaderSource)
		{
			...
			if (outputBinary[stage].size() == 0)
			{
				...
				// Compile shader
				{
					...
					{
						JM_CORE_ERROR(module.GetErrorMessage());
						JM_CORE_ASSERT(false);
					}
