...
namespace Jasmine {
	...
	static VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDebugReportCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
	{
		...
		JM_CORE_WARN("VulkanDebugCallback:\n  Object Type: {0}\n  Message: {1}", objectType, pMessage);
		return VK_FALSE;
	}
	...
	{
		JM_CORE_INFO("VulkanContext::Create");

		JM_CORE_ASSERT(glfwVulkanSupported(), "GLFW must support Vulkan!");
		...
		appInfo.pApplicationName = "Jasmine";
		appInfo.pEngineName = "Jasmine";
...
#define VK_KHR_WIN32_SURFACE_EXTENSION_NAME "VK_KHR_win32_surface"
		...
		if (s_Validation)
		{
			...
			JM_CORE_TRACE("Vulkan Instance Layers:");
			for (const VkLayerProperties& layer : instanceLayerProperties)
			{
				JM_CORE_TRACE("  {0}", layer.layerName);
			...
			{
				JM_CORE_ERROR("Validation layer VK_LAYER_LUNARG_standard_validation not present, validation is disabled");
			}
		}
		...
		if (s_Validation)
		{
			...
			JM_CORE_ASSERT(vkCreateDebugReportCallbackEXT != NULL, "");
