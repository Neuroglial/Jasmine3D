...
namespace Jasmine {
	...
	bool FileSystem::CreateFolder(const std::string& filepath)
	{
		...
		if (!created)
		{
			...
			if (error == ERROR_ALREADY_EXISTS)
				JM_CORE_ERROR("{0} already exists!", filepath);

			if (error == ERROR_PATH_NOT_FOUND)
				JM_CORE_ERROR("{0}: One or more directories don't exist.", filepath);
	...
	void FileSystem::StartWatching()
	{
		...
		JM_CORE_ASSERT(s_WatcherThread != NULL);
	}
	...
	unsigned long FileSystem::Watch(void* param)
	{
		...
		if (handle == INVALID_HANDLE_VALUE)
			JM_CORE_ERROR("Unable to accquire directory handle: {0}", GetLastError());

		overlapped.hEvent = CreateEvent(NULL, TRUE, FALSE, NULL);

		if (overlapped.hEvent == NULL)
		{
			JM_CORE_ERROR("CreateEvent failed!");
			return 0;
		}
