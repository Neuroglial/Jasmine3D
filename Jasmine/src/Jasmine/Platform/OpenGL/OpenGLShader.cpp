...
#if UNIFORM_LOGGING
#define JM_LOG_UNIFORM(...) JM_CORE_WARN(__VA_ARGS__)
#else
#define JM_LOG_UNIFORM
#endif
...
#define PRINT_SHADERS 1
	...
	void OpenGLShader::CompileOrGetOpenGLBinary(const std::unordered_map<uint32_t, std::vector<uint32_t>>& vulkanBinaries, bool forceCompile)
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			JM_CORE_ERROR("Shader compilation failed ({0}):\n{1}", m_AssetPath, &infoLog[0]);
		...
		for (auto& [bufferName, buffer] : m_Buffers)
		{
			for (auto& [name, uniform] : buffer.Uniforms)
			{
				...
				if (location == -1)
					JM_CORE_WARN("{0}: could not find uniform location {0}", name);
	...
	void OpenGLShader::Reflect(std::vector<uint32_t>& data)
	{
		...
		JM_CORE_TRACE("OpenGLShader::Reflect - {0}", m_AssetPath);
		JM_CORE_TRACE("   {0} Uniform Buffers", res.uniform_buffers.size());
		JM_CORE_TRACE("   {0} Resources", res.sampled_images.size());
		...
		for (const spirv_cross::Resource& resource : res.uniform_buffers)
		{
			...
			if (s_UniformBuffers.find(bindingPoint) == s_UniformBuffers.end())
			{
				...
				JM_CORE_TRACE("Created Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
			...
			else
			{
				...
				JM_CORE_ASSERT(buffer.Name == resource.name); // Must be the same buffer
				if (bufferSize > buffer.Size) // Resize buffer if needed
				{
					...
					JM_CORE_TRACE("Resized Uniform Buffer at binding point {0} with name '{1}', size is {2} bytes", buffer.BindingPoint, buffer.Name, buffer.Size);
				}
			}
		}
	...
	int32_t OpenGLShader::GetUniformLocation(const std::string& name) const
	{
		...
		if (result == -1)
			JM_CORE_WARN("Could not find uniform '{0}' in shader", name);
	...
	void OpenGLShader::SetUniformBuffer(const std::string& name, const void* data, uint32_t size)
	{
		...
		JM_CORE_ASSERT(uniformBuffer);
		JM_CORE_ASSERT(uniformBuffer->Size >= size);
	...
	void OpenGLShader::SetUniform(const std::string& fullname, float value)
	{
		Renderer::Submit([=]()
		{
			JM_CORE_ASSERT(m_UniformLocations.find(fullname) != m_UniformLocations.end());
