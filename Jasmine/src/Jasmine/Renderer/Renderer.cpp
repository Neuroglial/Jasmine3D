...
#include "Jasmine/Platform/OpenGL/OpenGLRenderer.h"
...
namespace Jasmine {
	...
	{
		JM_SCOPE_PERF("Renderer::WaitAndRender");
		s_CommandQueue->Execute();
	}

	void Renderer::BeginRenderPass(Ref<RenderPass> renderPass, bool clear)
	{
		JM_CORE_ASSERT(renderPass, "Render pass cannot be null!");
	...
	{
		JM_CORE_ASSERT(s_Data->UniformBuffers.find(frame) != s_Data->UniformBuffers.end());
		JM_CORE_ASSERT(s_Data->UniformBuffers.at(frame).find(set) != s_Data->UniformBuffers.at(frame).end());
		JM_CORE_ASSERT(s_Data->UniformBuffers.at(frame).at(set).find(binding) != s_Data->UniformBuffers.at(frame).at(set).end());
