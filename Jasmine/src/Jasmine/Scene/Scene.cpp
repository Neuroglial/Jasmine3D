...
namespace Jasmine {
	...
	static void OnScriptComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		JM_CORE_ASSERT(scene->m_EntityIDMap.find(entityID) != scene->m_EntityIDMap.end());
	...
	void Scene::OnRenderRuntime(Timestep ts)
	{
		...
		JM_CORE_ASSERT(cameraEntity, "Scene does not contain any cameras!");
	...
	void Scene::OnRuntimeStart()
	{
		...
		
		{
			...
			for (auto entity : view)
			{
				...
				*entityStorage = e;
				body->SetUserData(entityStorage);
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBody2DComponent>())
				{
					...
					JM_CORE_ASSERT(rigidBody2D.RuntimeBody);
	...
	Entity Scene::CreateEntityWithID(UUID uuid, const std::string& name, bool runtimeMap)
	{
		...
		JM_CORE_ASSERT(m_EntityIDMap.find(uuid) == m_EntityIDMap.end());
