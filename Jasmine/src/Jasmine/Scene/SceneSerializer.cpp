...
#include "Jasmine/Physics/PhysicsLayer.h"
#include "Jasmine/Physics/PXPhysicsWrappers.h"
#include "Jasmine/Renderer/MeshFactory.h"
...
namespace Jasmine {
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		JM_CORE_INFO("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				JM_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (transformComponent)
				{
					...
					{
						JM_CORE_ASSERT(rotationNode.size() == 3);
						transform.Rotation = transformComponent["Rotation"].as<glm::vec3>();
					}
					transform.Scale = transformComponent["Scale"].as<glm::vec3>();

					JM_CORE_INFO("  Entity Transform:");
					JM_CORE_INFO("    Translation: {0}, {1}, {2}", transform.Translation.x, transform.Translation.y, transform.Translation.z);
					JM_CORE_INFO("    Rotation: {0}, {1}, {2}", transform.Rotation.x, transform.Rotation.y, transform.Rotation.z);
					JM_CORE_INFO("    Scale: {0}, {1}, {2}", transform.Scale.x, transform.Scale.y, transform.Scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					JM_CORE_INFO("  Script Module: {0}", moduleName);
						...
						if (storedFields)
						{
							for (auto field : storedFields)
							{
								...
								switch (type)
								{
									...
									{
										JM_CORE_ASSERT(false, "Unimplemented");
										break;
									}
									case FieldType::Vec2:
									{
				...
				if (meshComponent)
				{
					...
					else
					{
						...
						if (filepath.empty())
							JM_CORE_ERROR("Tried to load non-existent mesh in Entity: {0}", deserializedEntity.GetUUID());
						else
							JM_CORE_ERROR("Tried to load invalid mesh '{0}' in Entity {1}", filepath, deserializedEntity.GetUUID());
					}
				}
				...
				if (skyLightComponent)
				{
					...
					else
					{
						...
						if (filepath.empty())
							JM_CORE_ERROR("Tried to load non-existent environment map in Entity: {0}", deserializedEntity.GetUUID());
						else
							JM_CORE_ERROR("Tried to load invalid environment map '{0}' in Entity {1}", filepath, deserializedEntity.GetUUID());
					}
				...
				if (boxColliderComponent)
				{
					...
					if (material)
					{
						...
						{
							JM_CORE_ERROR("Tried to load invalid Physics Material in Entity {0}", deserializedEntity.GetUUID());
						}
					}
				...
				if (meshColliderComponent)
				{
					...
					{
						JM_CORE_WARN("MeshColliderComponent in use without valid mesh!");
					}
