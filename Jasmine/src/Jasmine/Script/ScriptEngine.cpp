...
namespace Jasmine {
	...
	{
		JM_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	void ScriptEngine::LoadJasmineRuntimeAssembly(const std::string& path)
	{
		MonoDomain* domain = nullptr;
		bool cleanup = false;
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("Jasmine Runtime", nullptr);
			mono_domain_set(domain, false);
			
			cleanup = true;
		}

		s_CoreAssembly = LoadAssembly("assets/scripts/Jasmine-ScriptCore.dll");
	...
	{
		LoadJasmineRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadJasmineRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		JM_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		JM_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					JM_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					JM_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		JM_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		JM_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetJasmineFieldType(MonoType* monoType)
	{
		...
		switch (type)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (strcmp(name, "Jasmine.Vector2") == 0) return FieldType::Vec2;
				if (strcmp(name, "Jasmine.Vector3") == 0) return FieldType::Vec3;
				if (strcmp(name, "Jasmine.Vector4") == 0) return FieldType::Vec4;
			}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			JM_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	{
		JM_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		JM_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
